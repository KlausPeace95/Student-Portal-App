...
def notes(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            notes.save()
        messages.success(request, f"Note Added by {request.user.username} succesfully")
    else:
        form = NotesForm()
    form = NotesForm()
...
def dictionary(request):
    if request.method == "POST":
        ...
        # url = "https://od-api.oxforddictionaries.com/api/v2/entries/en_US" + text
        r = requests.get(url)
        answer = r.json()
        try:
...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request,f"Account created by {username} successfully!")
